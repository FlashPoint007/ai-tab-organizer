...
- 状态：**M5 发布准备完成**（lint / typecheck / 73 单测 / Chrome MV3 + Firefox 构建全绿）—— 计划与路线图见 [docs/PLAN.md](docs/PLAN.md)

## 特性

- ✅ 垂直标签侧边栏：搜索、列表/域名双视图、多选批量（关闭/固定/静音/建组）、一键清理（重复/非活跃，自动存快照）、快照恢复
- ✅ AI 自动分类：title+URL 批量送大模型 → 自动建 Chrome Tab Group；sha1 缓存省 token；模型没把握的自动回落规则
