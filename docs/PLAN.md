...
#### 反馈一：AI 分类要按内容细分，不要固定几类
- 提示词反转为「按内容细分」：4~12 类常见、类别名要具体（好例「B站内部平台」，坏例「网页」「其他」）、优先产品/平台/项目名、独特单标签可自成一类；类别上限 8→15
- **排障记录**：首轮反馈「还是宽泛分类」，根因是预览统计「LLM 1 · 缓存 12」——细分策略上线前的旧缓存把宽泛结果带回来了。缓存 key v1→v2 整体失效 + Options 新增「清空 AI 分类缓存」按钮 + 已有类别清单降为参考（明确宽泛旧类别不要硬套）

#### 反馈二：侧边栏同步分类
- 视图新增「分类」模式：按 Chrome 标签组名分区（组名异步查询+缓存），AI 整理完成后自动切换

#### M6 新功能
- **分类记忆（预览即学习）**：预览弹层勾选「记住调整」，确认时把手改的类别 diff 成域名规则沉淀（learning.ts 纯函数）。插入策略：同主机原地更新；**子域修正插到宽泛规则之前**（先匹配先赢，否则永不生效）；其余追加末尾。5 个单测
- **单标签实时归类**（realtimeClassifier.ts）：标签加载完成 2s 后按 缓存→规则→单条自适应LLM 求类别，并入同名组（无则建组，FNV-1a 稳定配色）；只碰未分组非固定标签；LLM 未配置静默降级；设置开关 realtime 默认开
- **备份导出导入**（backup.ts + Options 备份区）：JSON 备份（规则/分类/模型配置/界面偏好），zod 校验导入，API Key 默认不含需勾选；导出走 getExportBundle 协议
- 质量：89 单测全绿（新增 11 个），lint 0错0警，Chrome MV3 433KB / Firefox MV2 / zip 134KB
